...
router.delete('/agents/:id', requireAuth, requireRoles('SUPER_ADMIN','ADMIN'), async(req,res)=>{try{const updated=await repository.updateAgent(param(req,'id'),{status:'INACTIVE'},currentActor(req));if(!updated)return fail(res,'Agent not found',404);return ok(res,{deleted:true,agent:updated})}catch(e){return fail(res,e)}});
