...
router.get('/projects/:id', async(req,res)=>{try{const x=await repository.getProjectById(req.params.id)||await repository.getProjectBySlug(req.params.id);if(!x)return fail(res,'Project not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.get('/properties/:id', async(req,res)=>{try{const p=await repository.getPropertyById(req.params.id)||await repository.getPropertyBySlug(req.params.id); if(!p)return fail(res,'Property not found',404); return ok(res,p)}catch(e){return fail(res,e)}});
...
router.patch('/properties/:id', requireAuth, async(req,res)=>{try{const id=req.params.id;let updated;
...
router.delete('/properties/:id', requireAuth, requireRoles('SUPER_ADMIN','ADMIN'), async(req,res)=>{try{const d=await repository.deleteProperty(req.params.id,currentActor(req));if(!d)return fail(res,'Property not found',404);return ok(res,{deleted:true})}catch(e){return fail(res,e)}});
...
router.get('/agents/:slug', async(req,res)=>{try{const a=await repository.getAgentBySlug(req.params.slug);if(!a)return fail(res,'Agent not found',404);return ok(res,a)}catch(e){return fail(res,e)}});
...
router.patch('/inquiries/:id', requireAuth, async(req,res)=>{try{const x=await repository.updateInquiryStatus(req.params.id,req.body.status,req.body.notes);if(!x)return fail(res,'Inquiry not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.get('/leads/:id', requireAuth, async(req,res)=>{try{const x=await repository.getLeadById(req.params.id);if(!x)return fail(res,'Lead not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.patch('/leads/:id', requireAuth, async(req,res)=>{try{const x=req.body.stage?await repository.updateLeadStage(req.params.id,req.body.stage,req.body.note,currentActor(req)):await repository.updateLead(req.params.id,req.body,currentActor(req));if(!x)return fail(res,'Lead not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.patch('/viewings/:id', requireAuth, async(req,res)=>{try{const x=await repository.updateViewingStatus(req.params.id,req.body.status,req.body.notes,currentActor(req));if(!x)return fail(res,'Viewing not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.patch('/offers/:id', requireAuth, async(req,res)=>{try{const x=await repository.updateOfferStatus(req.params.id,req.body.status,req.body.notes,currentActor(req));if(!x)return fail(res,'Offer not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.patch('/deals/:id', requireAuth, async(req,res)=>{try{const x=await repository.updateDeal(req.params.id,req.body,currentActor(req));if(!x)return fail(res,'Deal not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.patch('/commissions/:id', requireAuth, async(req,res)=>{try{const x=await repository.updateCommissionStatus(req.params.id,req.body.status,req.body.paidDate,currentActor(req));if(!x)return fail(res,'Commission not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
...
router.get('/news/:slug', async(req,res)=>{try{const x=await repository.getNewsBySlug(req.params.slug);if(!x)return fail(res,'Article not found',404);return ok(res,x)}catch(e){return fail(res,e)}});
